...
export function keccak256Hex(data: Uint8Array): string {
  ...
  // Constants
  const RC = [
    1n, 32898n, 9223372036854808714n, 9223372039002292224n, 32907n,
    2147483649n, 9223372039002292353n, 9223372036854808585n, 138n, 136n,
    2147516425n, 2147483658n, 2147516555n, 9223372036854775947n,
    9223372036854808713n, 9223372036854808579n, 9223372036854775936n,
    32778n, 9223372036854808704n, 2147483649n, 9223372039002292353n,
    9223372039002292232n, 32907n, 9223372036854808579n
  ];
  const ROT = (
    [0, 36, 3, 41, 18],
    [1, 44, 10, 45, 2],
  ...
  for (let i = 0; i < bytes.length; i += rate) {
    for (let j = 0; j < rate; j += 8) {
      const idx = (i + j);
      let lo = 0n;
      for (let k = 0; k < 8; k++) lo |= BigInt(bytes[idx + k] || 0) << (8n * BigInt(k));
      s[j / 8] ^= lo;
    }
    // keccak-f[1600]
    for (let round = 0; round < 24; round++) {
      ...
      for (let i2 = 0; i2 < 25; i2 += 5) for (let x = 0; x < 5; x++) s[i2 + x] ^= D[x];
      let [x, y] = [1, 0];
      let current = s[1];
      for (let t = 0; t < 24; t++) {
        const X = y;
        ...
        const tmp = s[5 * y + x];
        s[5 * y + x] = ((current << BigInt(shift)) | (current >> BigInt(64 - shift))) & ((1n << 64n) - 1n);
        current = tmp;
      }
      for (let y2 = 0; y2 < 5; y2++) {
        const base = 5 * y2;
        const a0 = s[base + 0], a1 = s[base + 1], a2 = s[base + 2], a3 = s[base + 3], a4 = s[base + 4];
        s[base + 0] = a0 ^ (~a1 & a2);
        s[base + 1] = a1 ^ (~a2 & a3);
        s[base + 2] = a2 ^ (~a3 & a4);
        s[base + 3] = a3 ^ (~a4 & a0);
