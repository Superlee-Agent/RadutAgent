...
export const handleCheckIpAssets: RequestHandler = async (req, res) => {
  try {
    ...
    if (!/^0x[a-f0-9]{40}$/.test(trimmedAddress)) {
      console.error(`[IP Check] Invalid address format: "${address}" (trimmed: "${trimmedAddress}", length: ${trimmedAddress.length})`);
      return res.status(400).json({ error: "Invalid Ethereum address format. Expected 0x followed by 40 hexadecimal characters." });
    }
