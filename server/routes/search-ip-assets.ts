...
async function fetchParentIpDetails(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[Search IP] Failed to fetch parent details for ${ipId}: ${response.status}`,
      );
      return null;
    }
    ...
    console.log(
      `[Search IP] Fetched parent details for ${ipId}:`,
    ...
    console.warn(
      `[Search IP] Error fetching parent details for ${ipId}:`,
      error,
    );
    return null;
  }
